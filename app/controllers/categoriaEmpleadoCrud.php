...
switch ($action) {
    case 'get':
        $id = (int)appsec_clean_string(appsec_request_string('id_ai_categoria_empleado'));
        if ($id <= 0) {
            appsec_fail('ID invalido.', 400, ['error' => 'id_invalido']);
        }

        $row = $insEmpleado->obtenerCategoriaEmpleadoPorId($id);
        if (!$row) {
            ...
            if ((int)$exists['std_reg'] === 0) {
                $insEmpleado->ejecutarConsultaConParametros(
                    ...
                    [
                        ...
                        ':id' => (int)$exists['id_ai_categoria_empleado'],
                    ]
                );
    ...
    case 'delete':
        ...
        $inUse = $insEmpleado->ejecutarConsultaConParametros(
        ...
        if ($inUse && (int)$inUse->fetchColumn() > 0) {
            appsec_fail('No puedes eliminar una categoria con empleados activos asociados.', 400, ['error' => 'categoria_en_uso']);
        }
