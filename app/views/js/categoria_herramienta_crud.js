document.addEventListener('DOMContentLoaded', () => {
    ...
    const listContainer = document.getElementById('categoriaHerramientaListContainer');
    const btnRecargar = document.getElementById('btnRecargarCategoriaHerramienta');
    const formCreate = document.getElementById('formCategoriaHerramientaCreate');
    const formUpdate = document.getElementById('formCategoriaHerramientaUpdate');
    const modalEl = document.getElementById('ventanaModalModificarCategoriaHerramienta');

    if (!dir || !listContainer || !formCreate) return;

    const URL_CRUD = dir + 'app/controllers/categoriaHerramientaCrud.php';
    const URL_LIST = dir + 'app/controllers/categoriaHerramientaList.php';
    ...
    formCreate.addEventListener('submit', async (event) => {
        ...
        const nombre = (document.getElementById('nombre_categoria_herramienta')?.value || '').trim();
        if (!nombre) {
        ...
        const ok = await confirmDialog({
            ...
            text: 'Se registrara una nueva categoria de herramienta.',
    ...
    listContainer.addEventListener('click', async (event) => {
        const btnEdit = event.target.closest('.js-catherr-edit');
        const btnDel = event.target.closest('.js-catherr-del');
        ...
        if (btnDel) {
            ...
            const ok = await confirmDialog({
                ...
                text: 'La categoria se desactivara si no tiene herramientas activas asociadas.',
            ...
            fd.append('id_ai_categoria_herramienta', btnDel.getAttribute('data-id') || '');
        ...
        if (btnEdit) {
            ...
            fd.append('id_ai_categoria_herramienta', btnEdit.getAttribute('data-id') || '');
            ...
            document.getElementById('edit_id_ai_categoria_herramienta').value = resp.data.id_ai_categoria_herramienta || '';
            document.getElementById('edit_nombre_categoria_herramienta').value = resp.data.nombre_categoria || '';
            document.getElementById('edit_descripcion_categoria_herramienta').value = resp.data.descripcion || '';
