...
ALTER TABLE estado_ot
    ADD COLUMN IF NOT EXISTS libera_herramientas TINYINT(1) NOT NULL DEFAULT 0 AFTER color;

CREATE INDEX IF NOT EXISTS idx_estado_ot_libera_herramientas ON estado_ot (libera_herramientas, std_reg);

UPDATE estado_ot
SET libera_herramientas = 1
...
SET
    ot.ot_finalizada = CASE WHEN COALESCE(eo.libera_herramientas, 0) = 1 THEN 1 ELSE 0 END,
    ot.fecha_finalizacion = CASE
        WHEN COALESCE(eo.libera_herramientas, 0) = 1 THEN COALESCE(ot.fecha_finalizacion, NOW())
        ELSE NULL
    END,
    ot.id_user_finaliza = CASE
        WHEN COALESCE(eo.libera_herramientas, 0) = 1 THEN COALESCE(NULLIF(ot.id_user_finaliza, ''), ot.id_user)
        ELSE NULL
    END
WHERE ot.std_reg = 1;
...
BEGIN
  INSERT INTO log_user(
    ...
    'id_ai_estado,nombre_estado,color,libera_herramientas,std_reg',
    CONNECTION_ID(),
    USER(),
...
FOR EACH ROW
BEGIN
  IF COALESCE(OLD.libera_herramientas, 0) = 1
     AND (
    ...
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'El estado configurado para liberar herramientas es protegido y no puede modificarse ni eliminarse.';
  END IF;
END$$
